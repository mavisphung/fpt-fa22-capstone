...
class HealthRecordController extends GetxController with GetSingleTickerProviderStateMixin {
  ...
  List<InstructionType> lInstructionType = [];
  ...
  Future<bool> getInstructionType({int page = 1, int limit = 50}) async {
    final response = await _provider.getInstructionCategories(page: page, limit: limit);
    ...
    if (model.success == true) {
      ...
      for (var item in data) {
        result.add(InstructionType.fromMap(item));
        lInstructionType = result;
      }
      return true;
    }
    return false;
  ...
  Future<bool?> getHrWithId(int recordId) async {
    ...
    if (response?.isSuccess == true) {
      ...
      systemHrResModel = HrResModel.fromMap(data);
      return true;
  ...
  Future<void> getAllHealthRecords({int page = 1, int limit = 30}) async {
