...
class HomeController extends GetxController {
  ...
  RxList<Doctor> doctorBySpecList = <Doctor>[].obs;
  ...
  Future<bool> getDoctorListBySpecialist(int specId, {int page = 1, int limit = 10}) async {
    'Fetching doctors by specialist - Page $page - Limit $limit'.debugLog('HomeController');
    final result = await apiHome.getDoctorsBySpecialist(specId, page: page, limit: limit);
    final Map<String, dynamic> response = ApiResponse.getResponse(result);
    final model = ResponseModel2.fromMap(response);
    final data = model.data as List<dynamic>;
    doctorBySpecList.value += data.map((e) => Doctor.fromMap(e['doctor'])).toList();
    return true;
  }
  ...
  void dispose() {
    apiHome.dispose();
    doctorList.close();
    specialistList.close();
    nearestList.close();
    doctorBySpecList.close();
    super.dispose();
  }
}
